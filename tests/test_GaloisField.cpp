...
TEST_F(GaloisFieldTest, IdentityElements) {
    GaloisField::setPrime(7);

    GaloisField zero = GaloisField::zero();
    GaloisField one = GaloisField::one();

    EXPECT_EQ(zero.toInt(), 0);
    EXPECT_EQ(one.toInt(), 1);
...
TEST_F(GaloisFieldTest, UtilityFunctions) {
    GaloisField::setPrime(7);
    ...
    EXPECT_TRUE(zero.isZero());
    EXPECT_FALSE(other.isZero());

    EXPECT_TRUE(one.isOne());
    EXPECT_FALSE(other.isOne());

    EXPECT_EQ(other.toString(), "3");
...
TEST_F(GaloisFieldTest, FieldAxioms) {
    GaloisField::setPrime(5);

    for (int i = 1; i < 5; ++i) {
        GaloisField a = i;
        GaloisField inv = a.multiplicativeInverse();
        GaloisField product = a * inv;
        EXPECT_TRUE(product.isOne()) << "Multiplicative inverse failed for " << i;
    }

    for (int i = 0; i < 5; ++i) {
        GaloisField a = i;
        GaloisField inv = a.additiveInverse();
        GaloisField sum = a + inv;
        EXPECT_TRUE(sum.isZero()) << "Additive inverse failed for " << i;
    }

    GaloisField a = 2;
    GaloisField b = 3;
    EXPECT_TRUE(a + b == b + a) << "Addition not commutative";
    EXPECT_TRUE(a * b == b * a) << "Multiplication not commutative";

    GaloisField c = 4;
    EXPECT_TRUE((a + b) + c == a + (b + c)) << "Addition not associative";
    EXPECT_TRUE((a * b) * c == a * (b * c)) << "Multiplication not associative";

    EXPECT_TRUE(a * (b + c) == (a * b) + (a * c)) << "Distributivity failed";
}
